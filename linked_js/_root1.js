...
/* lib/yui/2.8.1/connection/connection.js */
...
(function(){LI.define("SignIn");
LI.SignIn=function(a){a=(typeof(a)=="string")?document.getElementById(a):a;
var c=YDom.getElementBy(function(i){return(i.type=="text")?true:false
},"input",a),j=YDom.getElementsBy(function(i){return(i.type=="text"||i.type=="password")
},"input",document),e=true,g=YDom.get("session_key-login"),b=YDom.get("email-coldRegistrationForm");
if(navigator.userAgent.indexOf("MSIE")<0){YDom.setAttribute(g,"type","email");
YDom.setAttribute(b,"type","email")
}var k=function(i){if(!YDom.hasClass(a,"show-sign-in")){if(i){var l=YEvent.getTarget(i)
}YDom.addClass(a,"show-sign-in");
c.focus();
YEvent.on(document,"click",f)
}};
var f=function(i){if(YDom.hasClass(a,"show-sign-in")){if(i){var m=YEvent.getTarget(i)
}else{l();
return
}function l(){YDom.removeClass(a,"show-sign-in");
YEvent.removeListener(document,"click",f)
}if(!YDom.isAncestor(a,m)){l()
}return
}};
for(var d=0,h;
h=j[d];
d++){YEvent.on(h,"focus",function(){e=true
});
YEvent.on(h,"blur",function(){e=false
})
}YEvent.on(a,"mouseover",k);
YEvent.on(document,"keypress",function(l){if(!e){var i=YEvent.getCharCode(l);
switch(i){case 115:YEvent.preventDefault(l);
k();
break;
case 47:YEvent.preventDefault(l);
var m=YDom.get("first");
m.focus();
break
}}})
};
YEvent.onDOMReady(function(){new LI.SignIn("sign-in-tab")
})
})();

/* js/photo_stack_show.js */

(function(){LI.define("StackShow");
